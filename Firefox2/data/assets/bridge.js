...
var BetterGaia = {
    ...
    localFile: function(url) {
		...
		catch(e) {console.log('[BetterGaia][Bridge] Error when emitting request for html: ' + e.message);}
    },
...
var Storage = {
    ...
    set: function(key, value) {
		...
		try {
            ...
            self.port.emit('set', [key, value]);
            self.port.on('set', function(data) {
                if (!data.success) throw new Error('Issue with chrome storage.');
				Storage.data[key] = value; // Sucess, put in bridge storage
            });
		}
		catch(e) {
